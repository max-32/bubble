...
class FacebookController extends Controller
{
    ...
    public static function generateAuthLink()
    {
        $params = array
        (
            'response_type' => self::FACEBOOK_RESPONSE_TYPE,
            'client_id' => self::FACEBOOK_CLIENT_ID,
            'redirect_uri' => self::FACEBOOK_REDIRECT_URI,
            'scope' => self::FACEBOOK_SCOPE,
            'v' => self::FACEBOOK_API_VERSION,
        );
    
        return (string) self::FACEBOOK_AUTH_PATH . '?' . urldecode(http_build_query($params));
    }
    ...
    {
        Log::info('Авторизация через Facebook прошла успешно.', $accessToken->toArray());
    ...
    protected function generateAccessTokenPath($temporaryVkCode)
    {
        ...
        return (string) self::FACEBOOK_ACCESS_TOKEN_PATH . '?' . urldecode(http_build_query($params));
    }
