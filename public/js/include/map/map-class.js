...
function DrawingManagerDataShell(dmanagerShell) {
	...
	function init(map, dmanager) {
		...
		// show button to remove overlay once it is selected
		// hide the button once no overlays are selected
		__toggleAddRemoveOverlayButton();
	}


	// internal system functions

	// add ability to show/hide "remove overlay" button
	function __toggleAddRemoveOverlayButton() {
		// shared memoy
		var controlElement = dmanagerShell.controls.create('<i class="fa fa-remove"></i>', 'Удалить выделенный элемент');

		// after overlay is selected or vice versa...
		self.bind('feature:selectchange', function(prev, current) {
			// here we got selected overlay, so the button to remove it
			// must be present
			if (current) {
				dmanagerShell.controls.remove( controlElement );
				dmanagerShell.controls.add( controlElement );

				$(controlElement.html).off('click');
				$(controlElement.html).on('click', function(event) {
					// remove overlay once button is clicked
					self.removeFeature( current );
					dmanagerShell.controls.remove( controlElement );
				});
			}
			// here we have no averlay selected
			// so the button must be hidden
			if (prev && ! current) {
				$(controlElement.html).off('click');
				dmanagerShell.controls.remove( controlElement );
			}
	...
	// toggle selected overlays on map
...
function ControlsManagerShell(dmanagerShell) {
	...
	// control panel, containin all the control elements
	this.controlsPanelVertical = __createControlsPanelVertical();


	ControlsManagerShell.position = {
		VERTICAL: 'vertical',
	...
	this.controls = {};
	// control elements
	this.controlsVertical = {};


	// create control element
	this.create = function(innerHTML, t) {
		let id = 'control_' + randomString(12);
		
		let controlElement = {
			id: id,
			html: __createControlElement(innerHTML, id, t),
			content: innerHTML,
		};
	...
	// add control element
	this.add = function(control, position) {
		if (position === ControlsManagerShell.position.VERTICAL) {
			this.controlsVertical[control.id] = ( control );
			__controlsPanelAddChild(this.controlsPanelVertical, control);
		} else {
	...
	this.remove = function(control) {
		...
		for (let currentControl in this.controlsVertical) {
			if (currentControl === control.id) {
				__controlsPanelRemoveChild(self.controlsPanelVertical, self.controlsVertical[currentControl]);
				delete self.controlsVertical[currentControl];
	...
	function __createControlsPanel() {
		var controlUI = document.createElement('div');
			controlUI.className = 'map-pane';

		return controlUI;
	}
	// create panel
	function __createControlsPanelVertical() {
		var controlUI = document.createElement('div');
			controlUI.className = 'map-pane map-pane-vertical';
	...
	function __createControlElement(innerHTML, id, t) {
		var itemOutter = document.createElement('div');
			itemOutter.className = 'map-item-outter';
		var item = document.createElement('div');
			item.className = 'map-item';
		var itemSpanContainer = document.createElement('span');
			itemSpanContainer.className = 'map-item-span-container';
		var itemSpanContainerImgHolder = document.createElement('div');
			itemSpanContainerImgHolder.className = 'map-item-span-container-imgholder';
		var itemSpanContainerImg = document.createElement('div');
			itemSpanContainerImg.className = 'map-item-span-container-img';
			itemSpanContainerImg.innerHTML = innerHTML;

			item.title = t ? t : '';
		
		itemSpanContainerImgHolder.appendChild( itemSpanContainerImg );
		itemSpanContainer.appendChild( itemSpanContainerImgHolder );
		item.appendChild( itemSpanContainer );
		itemOutter.appendChild( item );
	...
	function init(dmanagerShell) {
		dmanagerShell.map().controls[google.maps.ControlPosition.LEFT_CENTER].push( this.controlsPanelVertical );
